...
VANTA.register = (name, Effect) => {
    return VANTA[name] = async opts => {
        const instance = new Effect(opts)
        await instance.initThreeAsync()
        return instance
    }
}
...
VANTA.VantaBase = class VantaBase {
    ...
    async initThreeAsync() {
        ...
        // Add event listeners on window, because this element may be below other elements,
        // which would block the element's own mousemove event
