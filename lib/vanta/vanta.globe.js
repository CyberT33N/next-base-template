...
dracoLoader.setDecoderPath('https://www.gstatic.com/draco/versioned/decoders/1.5.6/') // Verwende die offiziellen Google-hosted Decoder
...
class Effect extends VantaBase {
    ...
    async loadCryptoLogo() {
        ...
        const scaleX = isMobile ? 1.0 : this.options.logo3dScaleX
        const scaleY = isMobile ? 1.0 : this.options.logo3dScaleY
        const scaleZ = isMobile ? 1.0 : this.options.logo3dScaleZ
        this.logo3d.scale.set(scaleX, scaleY, scaleZ)
    ...
    onWindowResize() {
        ...
        this.logo3d.visible = !isMobile // Set visibility based on device type

        const scale = isMobile ? 1.0 : 1.5 // Adjust scale for mobile
        this.logo3d.scale.set(scale, scale, scale)
    }
}
